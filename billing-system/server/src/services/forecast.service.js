...
import { sequelize, DemandForecast, Product } from '../models/index.js';
 ...
 */
export async function fetchSalesHistory({ branchId = null, productIds = null, from, to }) {
 ...
 */
export function forecastSeries(series, horizon = 1) {
...
export async function demandOverWindow({ productId, branchId, days }) {
  const today = new Date(toDateKey(new Date()));
