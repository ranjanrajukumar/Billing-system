...
export const setBusinessMode = asyncHandler(async (req, res) => {
  ...
  // Announced rather than acted on: whichever modules care about the mode
  // react for themselves. Accounting seeds its chart; nothing here knows that.
  await emit(POINTS.MODE_CHANGED, { mode });
...
export const saveCompany = asyncHandler(async (req, res) => {
  ...
  // The column holds three characters and the client formats money with Intl,
  // which rejects anything that is not a well-formed code. Refusing "Rs" here
  // is what stops it reaching the browser and blanking every screen that
  // shows a total.
  if (payload.currency !== undefined) {
    const code = String(payload.currency || '').trim().toUpperCase();
    if (!/^[A-Z]{3}$/.test(code)) {
      return res.status(400).json({
        message: 'Currency must be a three-letter code, such as INR, USD or EUR.',
      });
    }
