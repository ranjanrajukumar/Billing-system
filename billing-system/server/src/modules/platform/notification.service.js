...
export async function buildAlerts({ branchId = null, role = null, menus = [] } = {}) {
  ...
  // ---- Whatever the domains have to say ----
  //
  // The bell used to reach into inventory for the reconciliation figure, which
  // made the notification service — platform code — depend on a domain. The
  // domains now offer their alerts and this gathers them, so a module that is
  // not loaded contributes nothing instead of failing to import.
  //
  // They still go through `add`, because which alerts a given user may see is
  // this file's decision and not the contributor's.
  for (const alert of await emit(POINTS.ALERTS, { branchId })) add(alert);
