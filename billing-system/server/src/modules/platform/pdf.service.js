...
/**
 * How to label the currency on a printed invoice.
 *
 * The built-in PDF fonts encode WinAnsi, which has no rupee sign: ₹ resolves
 * to a zero-width glyph, so a bill that "shows" it in fact shows nothing at
 * all. Only symbols the font can genuinely draw are used; every other currency
 * — the rupee among them — falls back to its ISO code, which always renders.
 * Printing the symbol itself would mean bundling a Unicode font.
 */
const DRAWABLE_SYMBOLS = { USD: '$', EUR: '€', GBP: '£', JPY: '¥' };
const currencyOf = (invoice, company) =>
  String(invoice?.currency || company?.currency || 'INR').toUpperCase();
const currencyPrefix = (code) => DRAWABLE_SYMBOLS[code] || code + ' ';
...
export async function buildInvoicePdf(invoice, company, template = 'standard', title = 'TAX INVOICE') {
  ...
  // An invoice raised in another currency keeps it; otherwise the company default.
  const code = currencyOf(invoice, company);
  // The headline figure carries the currency. Line items stay bare numbers:
  // the thermal amount column is barely 18pt wide and a prefix would not fit.
  const grand = (n) => currencyPrefix(code) + money(n);
  ...
  return new Promise((resolve) => {
    ...
    if (tplName === 'dynamic' && config) {
      ...
      doc.fontSize(13).text(`Grand Total: ${grand(invoice.grandTotal)}`, { align: 'right' });
      ...
      doc.moveTo(margin, y).lineTo(rightX, y).stroke(); y += 5;
      doc.fontSize(10).text(`TOTAL (${code}):`, margin, y);
      ...
      doc.text(grand(invoice.grandTotal), 470, y + 42);
      ...
      doc.fontSize(14).text(`Grand Total: ${grand(invoice.grandTotal)}`, 350, y + 35, { align: 'right' });
      ...
      y += 10;
      doc.text(`Total: ${grand(invoice.grandTotal)}`, { align: 'right' });
