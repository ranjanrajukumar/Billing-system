...
import { SalesOrder, SalesOrderItem, Customer, Product, User, Invoice } from '../../models/index.js';
...
import { releaseReservation, reserveStock } from '../inventory/stock.service.js';
import { houseOwnerId } from '../warehouse/stockOwner.service.js';
import { invoicesFor, stockHasLeft } from './salesOrderLink.service.js';
...
export const getAll = asyncHandler(async (req, res) => {
  ...
  const { rows, count } = await SalesOrder.findAndCountAll({
    ...
    include: [
      ...
      // Whether an order has been billed is answered by looking at its
      // invoices, not by a flag on the order. A flag is a second copy of the
      // truth, and the copy is the one that goes stale.
      { model: Invoice, attributes: ['id', 'invoiceNumber', 'status', 'grandTotal'], required: false },
    ],
    limit,
    offset,
    order: [['addondt', 'DESC']]
...
export const getOne = asyncHandler(async (req, res) => {
  const item = await SalesOrder.findOne({
    ...
    include: [
      ...
      { model: Invoice, attributes: ['id', 'invoiceNumber', 'status', 'grandTotal', 'invoiceDate'], required: false },
    ]
  });
...
export const cancel = asyncHandler(async (req, res) => {
  ...
  // Cancelling an order whose goods have gone, or which has already been
  // billed, would leave the warehouse and the ledger describing a sale that
  // the order says never happened. Both have their own way back — a sales
  // return for the goods, a cancelled invoice for the money — and neither is
  // this one.
  if (stockHasLeft(order)) {
    return res.status(409).json({
      message: 'These goods have already been dispatched — raise a sales return instead of cancelling the order',
    });
  }
  const billed = await invoicesFor(order.id);
  if (billed.length) {
    return res.status(409).json({
      message: `Order ${order.orderNumber} is invoiced (${billed.map((i) => i.invoiceNumber).join(', ')}). Cancel the invoice first.`,
    });
  }
  ...
  await sequelize.transaction(async (t) => {
    ...
    //
    // `confirm` records that by setting the status to Approved. This read
    // 'Confirmed' — a value the status enum does not contain and nothing ever
    // wrote — so the branch never ran and every cancelled order left its
    // reservation behind: stock on the shelf that no order was waiting for and
    // nobody could sell, with nothing on screen to say why.
