...
// A browser can draw any of these, so the HTML invoice shows the real symbol.
// The PDF cannot — its built-in fonts have no rupee glyph — and falls back to
// ISO codes instead; see pdf.service.js.
const SYMBOLS = {
  INR: '₹', USD: '$', EUR: '€', GBP: '£', JPY: '¥',
  AED: 'AED ', AUD: 'A$', CAD: 'C$', SGD: 'S$',
};

/** The bill's own currency where it has one, else the company default. */
const symbolFor = (invoice, company) => {
  const code = String(invoice?.currency || company?.currency || 'INR').toUpperCase();
...
function renderItems(block, { invoice, company }) {
  const symbol = symbolFor(invoice, company);
  ...
  const columns = [
    ...
    withAmounts && { header: 'Rate', value: (it) => money(it.rate, symbol), num: true },
    withAmounts && block.showDiscount && { header: 'Disc.', value: (it) => money(it.discount, symbol), num: true },
    withAmounts && block.showGst && { header: 'GST %', value: (it) => `${Number(it.gstPercent || 0)}%`, num: true },
    withAmounts && { header: 'Amount', value: (it) => money(it.amount, symbol), num: true },
  ].filter(Boolean);
...
function renderTotals(block, { invoice, company }) {
  const symbol = symbolFor(invoice, company);
  const lines = [`<div><span>Subtotal</span><span>${money(invoice.subtotal, symbol)}</span></div>`];
  if (block.showTaxBreakup) {
    if (Number(invoice.igst) > 0) {
      lines.push(`<div><span>IGST</span><span>${money(invoice.igst, symbol)}</span></div>`);
    } else {
      lines.push(`<div><span>CGST</span><span>${money(invoice.cgst, symbol)}</span></div>`);
      lines.push(`<div><span>SGST</span><span>${money(invoice.sgst, symbol)}</span></div>`);
    }
  }
  if (block.showRoundOff) lines.push(`<div><span>Round Off</span><span>${money(invoice.roundOff, symbol)}</span></div>`);
  lines.push(`<div class="grand"><span>Grand Total</span><span>${money(invoice.grandTotal, symbol)}</span></div>`);
