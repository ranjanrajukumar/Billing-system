...
import { pick, returnToBins, suggestPick, usesBins } from './binStock.service.js';
import { getBranchStock, postStockTransaction, releaseReservation } from '../inventory/stock.service.js';
import { deriveStatus, progressOf, remainingOn } from './fulfilment.service.js';
import { buildRoute, createPickTasks, routeDto } from './pickPath.service.js';
...
export const dispatch = asyncHandler(async (req, res) => {
  const result = await sequelize.transaction(async (transaction) => {
    ...
    const branchId = order.fulfilFromBranchId || req.branchId;
    // Where the hold was put, which is the order's own branch — not necessarily
    // the one it is shipping from.
    const reservedAt = order.branchId || branchId;
    let moved = 0;
    ...
    for (const item of order.SalesOrderItems) {
      ...
      // Release the hold first, then move the stock.
      //
      // Both halves are needed and the order is not arbitrary. A confirmed
      // order reserves its quantity, and availability is stock minus what is
      // reserved — so deducting while the hold is still on measures this
      // order's own goods as unavailable and refuses to ship them, which on a
      // fully-committed line means the order can never leave the building.
      // Releasing afterwards instead would leave the deduction failing for the
      // same reason. Releasing first frees exactly this order's claim, and the
      // two writes are in one transaction, so no other till sees the gap.
