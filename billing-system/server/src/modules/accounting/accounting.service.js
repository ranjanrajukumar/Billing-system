...
import { withoutAudit } from '../platform/audit.service.js';
...
/**
 * Material issued out of the store: the cost moves from inventory to expense.
 *
 * Dr Materials Consumed / Cr Inventory. Nothing here is revenue and nothing is
 * GST — no customer received these goods, so there is no supply to tax and
 * nothing to bill. That is exactly what separates this from a sale, and it is
 * why an issue posted as a sale at zero value would quietly report a hundred
 * per cent loss on every line.
 *
 * The whole issue is treated as consumed on the way out, including any part
 * that later comes back. That is not an approximation to be corrected: on the
 * day of the issue, consumed is what it is. The return, when and if it happens,
 * is its own event on its own date, and posting it that way is what stops a
 * month's figures changing after the month has been reported.
 */
export async function postStockIssue({ issue, consumedValue = 0, transaction, userId }) {
  const consumed = money(consumedValue);
  ...
  return postIfEnabled({
    ...
    narration: `${issue.purpose} — ${issue.issueNumber}`,
    lines: [
      { code: ACCOUNTS.MATERIALS_CONSUMED, debit: consumed, credit: 0 },
      { code: ACCOUNTS.INVENTORY, debit: 0, credit: consumed },
    ],
    userId,
    transaction,
  });
...
/**
 * Material coming back to the store: the cost moves back out of expense.
 *
 * The mirror of the entry above, and only for the part that is fit to use —
 * goods returned damaged stay an expense, because they are. They move from
 * Materials Consumed to Inventory Write-off rather than back to inventory,
 * which is where they actually belong: spent, but not on anything useful.
 */
export async function postIssueReturn({ issueReturn, issue, goodValue = 0, scrapValue = 0, transaction, userId }) {
  const good = money(goodValue);
  const scrapped = money(scrapValue);
  if (good === 0 && scrapped === 0) return null;

  const lines = [];
  if (good > 0) {
    lines.push({ code: ACCOUNTS.INVENTORY, debit: good, credit: 0 });
  }
  if (scrapped > 0) {
    lines.push({ code: ACCOUNTS.INVENTORY_WRITE_OFF, debit: scrapped, credit: 0 });
  }
  lines.push({ code: ACCOUNTS.MATERIALS_CONSUMED, debit: 0, credit: money(good + scrapped) });

  return postIfEnabled({
    date: issueReturn.returnDate,
    branchId: issueReturn.branchId,
    sourceType: 'StockIssueReturn',
    sourceId: issueReturn.id,
    sourceNumber: issueReturn.returnNumber,
    narration: `Returned against ${issue.issueNumber} — ${issueReturn.returnNumber}`,
    lines,
    userId,
    transaction,
  });
