...
export default function Expenses() {
  ...
  // Category and location are optional to the API and not to the books: an
  // expense without them cannot be reported on or posted anywhere sensible.
  const expenseFields = useRequiredFields([
    { name: 'categoryId', label: 'Category' },
    { name: 'branchId', label: 'Location' },
    { name: 'expenseDate', label: 'Date' },
  ...
  const save = async () => {
    if (!expenseFields.check(editing, showToast)) return;
          ...
          <Stack spacing={2}>
            <Grid container spacing={1.5}>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small" label="Category" {...expenseFields.fieldProps('categoryId', editing)} value={editing.categoryId || ''}
                  onChange={(e) => setEditing({ ...editing, categoryId: e.target.value })} InputLabelProps={{ shrink: true }}>
                  {categories.map((c) => <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>)}
                </TextField>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small" label="Location" {...expenseFields.fieldProps('branchId', editing)} value={editing.branchId || ''}
              ...
              <Grid item xs={12} sm={6}>
                <TextField fullWidth size="small" type="date" label="Date" {...expenseFields.fieldProps('expenseDate', editing)} value={editing.expenseDate || ''}
              ...
              <Grid item xs={6}>
                <TextField fullWidth size="small" type="number" label="Amount" {...expenseFields.fieldProps('amount', editing)} value={editing.amount ?? ''}
