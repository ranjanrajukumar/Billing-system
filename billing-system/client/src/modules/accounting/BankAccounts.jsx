...
export default function BankAccounts() {
  ...
  // An account with no name is unidentifiable on every screen that lists it;
  // the bank and account number genuinely are optional, because a petty-cash
  // ledger kept as an "account" has neither.
  const accountFields = useRequiredFields([{ name: 'accountName', label: 'Account name' }]);
  ...
  const openBlank = () => { accountFields.reset(); setEditing({
            ...
            <Grid container spacing={1.5}>
              <Grid item xs={12} sm={6}>
                <TextField fullWidth size="small" label="Account name" value={editing.accountName || ''}
                  {...accountFields.fieldProps('accountName', editing)}
            ...
            <Paper variant="outlined" sx={{ borderRadius: 2, p: 1.5 }}>
              <Grid container spacing={1} alignItems="center">
                <Grid item xs={12} sm={3}>
                  <TextField select fullWidth size="small" label="Type" required value={entry.entryType}
                ...
                <Grid item xs={6} sm={2}>
                  {/* The save is already guarded on a positive amount; this is
                      the asterisk that was missing from the guard. */}
                  <TextField fullWidth size="small" type="number" label="Amount" required value={entry.amount}
