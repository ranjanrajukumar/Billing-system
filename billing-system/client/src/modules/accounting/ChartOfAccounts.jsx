...
export default function ChartOfAccounts() {
  ...
  // Code, name, type and side. An account missing any of them cannot be posted
  // to or read on a statement.
  const accountFields = useRequiredFields([
    { name: 'code', label: 'Code' },
    { name: 'name', label: 'Name' },
    { name: 'accountType', label: 'Type' },
    { name: 'normalBalance', label: 'Normal balance' },
  ]);
  ...
  const save = async () => {
    if (!accountFields.check(editing, showToast)) return;
          ...
          <Stack spacing={2}>
            ...
            <Grid container spacing={1.5}>
              <Grid item xs={12} sm={4}>
                <TextField fullWidth size="small" label="Code" {...accountFields.fieldProps('code', editing)} value={editing.code || ''} disabled={editing.isSystem}
                  onChange={(e) => setEditing({ ...editing, code: e.target.value })} InputLabelProps={{ shrink: true }} />
              </Grid>
              <Grid item xs={12} sm={8}>
                <TextField fullWidth size="small" label="Name" {...accountFields.fieldProps('name', editing)} value={editing.name || ''}
                  onChange={(e) => setEditing({ ...editing, name: e.target.value })} InputLabelProps={{ shrink: true }} />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small" label="Type" {...accountFields.fieldProps('accountType', editing)} value={editing.accountType || 'Expense'} disabled={editing.isSystem}
              ...
              <Grid item xs={12} sm={6}>
                <TextField select fullWidth size="small" label="Normal balance" {...accountFields.fieldProps('normalBalance', editing)} value={editing.normalBalance || 'Debit'} disabled={editing.isSystem}
