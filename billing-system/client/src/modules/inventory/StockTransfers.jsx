...
export default function StockTransfers() {
  ...
  // Both ends and a date. The API already demands the two locations; the form
  // never said so.
  const transferFields = useRequiredFields([
    { name: 'fromBranchId', label: 'From location' },
    { name: 'toBranchId', label: 'To location' },
    { name: 'transferDate', label: 'Transfer date' },
  ]);
  ...
  const submit = async () => {
    if (!transferFields.check(form, showToast)) return;
        ...
        <Stack spacing={2}>
          <Grid container spacing={1.5}>
            <Grid item xs={12} sm={6}>
              <TextField
                select fullWidth size="small" label="From" {...transferFields.fieldProps('fromBranchId', form)} value={form.fromBranchId}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                select fullWidth size="small" label="To" {...transferFields.fieldProps('toBranchId', form)} value={form.toBranchId}
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                fullWidth size="small" type="date" label="Date" {...transferFields.fieldProps('transferDate', form)} value={form.transferDate}
