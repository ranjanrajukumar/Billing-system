...
export default function StockCounts() {
  ...
  // A count is of one location on one day. Missing either, there is nothing to
  // compare the result against.
  const countFields = useRequiredFields([
    { name: 'branchId', label: 'Location' },
    { name: 'countDate', label: 'Count date' },
  ]);
  ...
  const start = async () => {
    if (!countFields.check(form, showToast)) return;
    ...
    <Stack spacing={3} className="animate-fadeInUp">
      ...
      <Modal open={creating} title="Start a Stock Count" onClose={() => setCreating(false)} maxWidth="sm">
        <Stack spacing={2}>
          ...
          <TextField
            select fullWidth size="small" label="Location" {...countFields.fieldProps('branchId', form)} value={form.branchId}
          ...
          <TextField
            fullWidth size="small" type="date" label="Count date" {...countFields.fieldProps('countDate', form)} value={form.countDate}
