...
export default function Serials() {
  ...
  // A serial belongs to a product at a location. Without both there is nothing
  // for the number to identify.
  const serialFields = useRequiredFields([
    { name: 'productId', label: 'Product' },
    { name: 'branchId', label: 'Location' },
    { name: 'serialNumbers', label: 'Serial numbers' },
  ]);
  ...
  const add = async () => {
    if (!serialFields.check(form, showToast)) return;
  ...
  return (
    <Stack spacing={3} className="animate-fadeInUp">
      ...
      <Modal open={creating} title="Add Serial Numbers" onClose={() => setCreating(false)} maxWidth="sm">
        <Stack spacing={2}>
          <TextField select fullWidth size="small" label="Product" {...serialFields.fieldProps('productId', form)} value={form.productId}
            onChange={(e) => setForm({ ...form, productId: e.target.value })} InputLabelProps={{ shrink: true }}>
            {products.map((p) => <MenuItem key={p.id} value={p.id}>{p.productName}</MenuItem>)}
          </TextField>
          <TextField select fullWidth size="small" label="Location" {...serialFields.fieldProps('branchId', form)} value={form.branchId}
          ...
          <TextField
            ...
            {...serialFields.fieldProps('serialNumbers', form)}
