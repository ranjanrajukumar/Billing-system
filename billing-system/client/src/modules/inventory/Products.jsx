...
import UnitsAndPacks from './UnitsAndPacks.jsx';
...
export default function Products() {
  ...
  // The product whose units and packs are open, or null.
  ...
  const openForm = (row = null) => {
    ...
    reset({
      ...
      isActive: row.isActive !== false,      customAttributes: row.customAttributes || {},
    });
  };
  ...
  return (
    <Stack spacing={3} className="animate-fadeInUp">
            ...
            columns={[
              ...
              { field: 'actions', headerName: 'Actions', render: (row) => (
                <Stack direction="row" spacing={0.5}>
                  ...
                  {/* Only for a saved product: units and packs hang off an id. */}
                  <Tooltip title="Units & Packs"><IconButton size="small" onClick={() => setPackaging(row)} sx={{ borderRadius: 1.5, color: 'info.main', bgcolor: alpha(theme.palette.info.main, 0.08), '&:hover': { bgcolor: alpha(theme.palette.info.main, 0.15) } }}><WidgetsIcon fontSize="small" /></IconButton></Tooltip>
      ...
      {/* Units and packs — the two ways one product comes in more than one size */}
      <Modal
        open={Boolean(packaging)}
        title={`Units & Packs — ${packaging?.productName || ''}`}
        onClose={() => setPackaging(null)}
        maxWidth="md"
      >
        {packaging && <UnitsAndPacks product={packaging} />}
      </Modal>
      ...
      <Modal open={Boolean(editing)} title={editing?.id ? 'Update Product' : 'Add Product'} onClose={() => setEditing(null)}>
        <Grid container spacing={2} component="form" onSubmit={handleSubmit(submit)} sx={{ pt: 0.5 }}>
          ...
          <Grid item xs={12} sm={6}><TextField fullWidth label="Product Name *" required {...register('productName', { required: 'Product name is required' })} error={Boolean(errors.productName)} helperText={errors.productName?.message} InputLabelProps={{ shrink: true }} /></Grid>
