...
export default function PurchaseOrders() {
  ...
  // An order commits the business to a supplier on a date, for delivery
  // somewhere. The expected date stays optional — often it is what you are
  // asking the supplier for.
  const poFields = useRequiredFields([
    { name: 'poDate', label: 'Order date' },
    { name: 'branchId', label: 'Deliver to' },
  ]);
  ...
  const submitForm = async () => {
    if (!poFields.check(editing, showToast)) return;
        ...
        {editing && (
          <Stack spacing={2}>
            <Grid container spacing={1.5}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  select fullWidth size="small" label="Deliver to" {...poFields.fieldProps('branchId', editing)} value={editing.branchId || ''}
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  fullWidth size="small" type="date" label="Order date" {...poFields.fieldProps('poDate', editing)} value={editing.poDate || ''}
