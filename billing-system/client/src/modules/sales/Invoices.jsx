...
const blankItem = { productId: '', variantId: null, quantity: 1, rate: 0, discount: 0, gstPercent: 18, packing: '', um: '', batchId: '' };
  ...
  const openEdit = async (row) => {
    try {
      ...
      setItems((invoice.InvoiceItems || []).map((line) => ({
        ...
        // Which pack it sold, so reopening a bill shows what was actually sold
        // rather than silently rewriting it as loose stock on the next save.
        ...
        <>
          <DataTable
            ...
            columns={[
              ...
              // The order this bill came from, when it came from one. Worth a
              // line of its own rather than only being printed on the PDF: a
              // bill against a dispatched order deliberately moves no stock,
              // and this is the only thing on screen that explains why.
              { field: 'salesOrder', headerName: 'Order', render: (row) => (
                row.salesOrderId
                  ? <Chip label={row.orderNumber || `SO #${row.salesOrderId}`} size="small" color="info" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.7rem' }} />
        ...
        <Stack spacing={3} component="form" onSubmit={handleSubmit(submit)}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={4}>
              <TextField fullWidth type="date" label="Invoice Date" InputLabelProps={{ shrink: true }} required {...register('invoiceDate', { required: true })} />
            </Grid>
            <Grid item xs={12} sm={4}>
          ...
          <Paper variant="outlined" sx={{ borderRadius: 2.5, overflow: 'hidden' }}>
            ...
            <Box sx={{ p: 2 }}>
              {/* The shared editor. Everything this page used to re-implement
                  — product picking, the sell-as dropdown, packs, lots, the
                  per-line total — now lives in one component, so the invoice
                  screen cannot drift from the other documents again. */}
              <LineItems
                items={items}
                onChange={setItems}
                products={products}
                blank={blankItem}
                lots={batchesByProduct}
                onProductChosen={loadBatches}
                showPacking
                lineTotal={(item) => currency(lineTotal(item))}
                productDisabled={(p) => Number(p.stock || 0) <= 0}
                productMeta={(p) => `Stock: ${p.stock} · HSN: ${p.hsnCode || '—'}`}
              />
            </Box>
          </Paper>
