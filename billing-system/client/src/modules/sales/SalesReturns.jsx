...
import LineItems, { incompleteLines } from '../../components/LineItems.jsx';
...
export default function SalesReturns() {
  ...
  // Set on the first save attempt: a blank row that has only just been
  // added is not yet a mistake.
  ...
  const submit = async (values) => {
    setLineErrors(true);
    if (incompleteLines(items).length) {
        ...
        <Stack spacing={2.5} component="form" onSubmit={handleSubmit(submit)}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={3}>
              <TextField select fullWidth label="Customer" defaultValue="" required {...register('customerId', { required: true })} InputLabelProps={{ shrink: true }}>
            ...
            <Grid item xs={12} sm={3}>
              <TextField fullWidth type="date" label="Return Date" required {...register('returnDate', requiredRule('Return date'))} InputLabelProps={{ shrink: true }} />
            </Grid>
            <Grid item xs={12} sm={3}>
          ...
          <LineItems
            showErrors={lineErrors} items={items} onChange={setItems} products={products} fields={['rate']} blank={blankItem} />
