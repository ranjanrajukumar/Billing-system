...
/**
 * The bills standing against an order.
 *
 * A cancelled invoice is not one: cancelling it put the stock back, so the
 * order is billable again and the screen should say so.
 */
const liveInvoices = (order) => (order.Invoices || []).filter((i) => i.status !== 'Cancelled');
...
export default function SalesOrders() {
   ...
   * Bill the order.
   *
   * Which quantities end up on the bill is the server's call, not this
   * screen's: before the warehouse dispatches, an order bills what was
   * ordered; afterwards it bills what actually went, so a short shipment bills
   * short. Sending nothing but the id is what keeps the two from disagreeing.
   */
  const raiseInvoice = async (order) => {
    try {
      const invoice = await salesOrdersApi.invoice(order.id);
      showToast(`Invoice ${invoice.invoiceNumber} raised for ${order.orderNumber}`, 'success');
      load();
    } catch (err) {
      showToast(err.response?.data?.message || 'Could not raise the invoice', 'error');
    }
  };
        ...
        <>
          <DataTable
            ...
            columns={[
              ...
              // Billed or not, read off the order's invoices rather than a flag,
              // so the column cannot claim something the ledger disagrees with.
              { field: 'billing', headerName: 'Billing', render: (r) => (
                liveInvoices(r).length
                  ? <Chip label={liveInvoices(r).map((i) => i.invoiceNumber).join(', ')} size="small" color="success" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.7rem' }} />
                  : <Chip label="Not billed" size="small" color="default" variant="outlined" sx={{ fontSize: '0.7rem' }} />
              ) },
              ...
              { field: 'actions', headerName: 'Actions', render: (r) => (
                <Stack direction="row" spacing={0.25}>
                  ...
                  {/* Raise the bill — once, and not for a cancelled order.
                      A second bill is refused by the server too; hiding the
                      button as well just saves the round trip. */}
                  {!liveInvoices(r).length && r.status !== 'Cancelled' && (
                    <Tooltip title="Raise Invoice for this Order">
                      <IconButton size="small" color="primary" onClick={() => raiseInvoice(r)} sx={{ borderRadius: 1.5 }}>
        ...
        <Stack spacing={3} component="form" onSubmit={handleSubmit(submit)}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={4}>
              <TextField fullWidth type="date" label="Order Date" InputLabelProps={{ shrink: true }} required {...register('orderDate', { required: true })} />
            </Grid>
            <Grid item xs={12} sm={4}>
              <Controller
                name="customerId"
