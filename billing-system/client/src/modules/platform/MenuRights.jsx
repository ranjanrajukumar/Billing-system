...
export default function MenuRights() {
  ...
  /**
   * The grantable entries under an item.
   *
   * A process is not one of them. It has a page, but it is not a right: the
   * server shows it exactly when one of its documents is visible. Offering a
   * tick box for it would invite somebody to grant the overview of a flow whose
   * documents they have just unticked, and then wonder why it vanished.
   */
  const leavesOf = (item) => (item.children || [item]);
  const leavesIn = (items) => items.flatMap(leavesOf);

  const toggleGroup = (items, on) => setSelected((prev) => {
    const keys = leavesIn(items).map((i) => i.key);
  ...
  const totalSelected = useMemo(
    () => catalogue
      ...
      .filter((i) => selected.includes(i.key)).length,
    [catalogue, selected],
  );
  ...
  return (
    <Stack spacing={2}>
      ...
      <Grid container spacing={2}>
        {catalogue.map((group) => {
          const keys = leavesIn(group.items).map((i) => i.key);
              ...
              <Paper variant="outlined" sx={{ borderRadius: 2.5, p: 2, height: '100%' }}>
                ...
                <Stack>
                  {group.items.map((item) => {
                    const tick = (entry) => {
                      const locked = alwaysVisible.includes(entry.key);
                      return (
                        <FormControlLabel
                          key={entry.key}
                          control={
                            <Checkbox
                              size="small"
                              checked={selected.includes(entry.key) || locked}
                    ...
                    if (!item.children) return tick(item);

                    // A process reads as a heading over its documents, with no
                    // tick of its own — granting one of the documents is what
                    // makes the flow visible.
                    return (
                      <Box key={item.key} sx={{ mt: 0.5 }}>
                        <Typography
                          variant="caption"
                          sx={{
