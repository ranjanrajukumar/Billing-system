...
export default function Settings() {
  ...
  // A dropdown rather than free text: the field is three characters wide in the
  // database and Intl rejects anything that is not a real code, so "Rs" typed
  // here used to save cleanly and then break every screen that shows money.
  const CURRENCIES = [
  ...
  const companyFields = [
    ...
    { name: 'currency', label: 'Currency', sm: 4, required: true, options: CURRENCIES },
    ...
    <Stack spacing={3} component="form" onSubmit={handleSubmit(submit)} className="animate-fadeInUp">
      ...
      <SectionCard title="Company Information" icon={<BusinessIcon fontSize="small" />}>
        <Grid container spacing={2}>
          {companyFields.map(({ name, label, sm, required, options }) => (
            <Grid item xs={12} sm={sm} key={name}>
              {options ? (
                // A select is driven by its `value`, and register() supplies
                // none — so the box rendered empty however many times reset()
                // ran with the saved code. Same trap as the branch switch below.
                <Controller
                  name={name}
                  control={control}
                  defaultValue=""
