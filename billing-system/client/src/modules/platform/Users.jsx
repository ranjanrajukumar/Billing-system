...
import MenuRights from './MenuRights.jsx';
import LocationRights from './LocationRights.jsx';
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Stack spacing={2.5} sx={{ pt: 0.5 }}>
            <TextField label="Role Name" fullWidth disabled={editing?.name === 'Admin'}
              required {...register('name', { required: 'Role name is required' })} error={Boolean(errors.name)} helperText={errors.name?.message} />
  ...
  return (
    <Stack spacing={2}>
      ...
      <Modal open={open} onClose={handleClose} title={editing ? 'Edit User' : 'Add User'}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <Stack spacing={2.5} sx={{ pt: 0.5 }}>
            <TextField label="Full Name" fullWidth required {...register('name', { required: 'Required' })} error={Boolean(errors.name)} helperText={errors.name?.message} InputLabelProps={{ shrink: true }} />
            <TextField label="Email" type="email" fullWidth required {...register('email', { required: 'Required' })} error={Boolean(errors.email)} helperText={errors.email?.message} InputLabelProps={{ shrink: true }} />
            <TextField label="Mobile" fullWidth {...register('mobile')} InputLabelProps={{ shrink: true }} />
            {/* Required only when creating: editing leaves it blank to keep the
                existing password, so an unconditional asterisk would be a lie
                on half the times this form opens. */}
            <TextField label="Password" type="password" fullWidth required={!editing} {...register('password', { required: !editing && 'Required', minLength: 6 })} error={Boolean(errors.password)} helperText={errors.password?.message || (editing ? 'Leave blank to keep unchanged' : '')} InputLabelProps={{ shrink: true }} />
            <TextField select label="Role" fullWidth required {...register('roleId', { required: 'Required' })} error={Boolean(errors.roleId)} helperText={errors.roleId?.message} defaultValue="">
